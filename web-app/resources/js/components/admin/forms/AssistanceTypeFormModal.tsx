...
interface ModalProps {
    isOpen: boolean;
    initialData: AssistanceTypeItem | null;
    categories: { id: number; name: string }[];
    onClose: () => void;
}

export default function AssistanceTypeFormModal({ isOpen, initialData, categories, onClose }: ModalProps) {
...
return null;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEdit && initialData?.id) {
            put(`/admin/assistance-types/${initialData.id}`, { onSuccess: () => {
 reset(); onClose(); 
} });
        } else {
            post('/admin/assistance-types', { onSuccess: () => {
 reset(); onClose(); 
} });
        }
    };
        ...
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm overflow-y-auto">
            <div className="w-full max-w-lg rounded-xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl my-8">
                <header className="flex items-center justify-between border-b border-zinc-800 pb-4">
                    <div className="flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-red-500" /><h2 className="text-lg font-bold text-white">{isEdit ? 'Edit Jenis Bantuan' : 'Tambah Jenis Bantuan'}</h2></div>
                ...
                <form onSubmit={handleSubmit} className="mt-5 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-zinc-300">Label Bantuan *</label>
                            <input type="text" required value={data.label} onChange={(e) => setData('label', e.target.value)} className="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-950 py-2 px-3 text-sm text-zinc-100" placeholder="Ambulans / Tim Medis" />
                        ...
                        <div>
                            ...
                            <input type="text" required value={data.code} onChange={(e) => setData('code', e.target.value.toUpperCase())} className="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-950 py-2 px-3 text-sm font-mono text-zinc-100 uppercase" placeholder="ASST_AMBULANCE" />
                            {errors.code && <p className="text-xs text-red-400 mt-1">{errors.code}</p>}
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-zinc-300">Fragmen Template *</label>
                        <input type="text" required value={data.template_fragment} onChange={(e) => setData('template_fragment', e.target.value)} className="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-950 py-2 px-3 text-sm font-mono text-blue-300" placeholder="membutuhkan pengiriman ambulans segera" />
                    ...
                    <footer className="flex justify-end gap-3 pt-4 border-t border-zinc-800">
                        ...
                        <button type="submit" disabled={processing} className="min-h-[44px] px-6 rounded-lg bg-red-600 text-sm font-semibold text-white hover:bg-red-500">Simpan Jenis Bantuan</button>
                    </footer>
                </form>
            </div>
        </div>
